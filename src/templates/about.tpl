<div class="aside aside-right">
  <h2>Affiliations</h2>
</div>
<div class="honeycomb">
  <div class="honeycomb-row">
    <div class="honeycomb-cell"></div>
    <div class="honeycomb-cell"></div>
    <div class="honeycomb-cell"></div>
    <div class="honeycomb-cell"></div>
    <div class="honeycomb-cell"></div>
    <div class="honeycomb-cell"></div>
    <div class="honeycomb-cell"></div>
    <div class="honeycomb-cell"></div>
    <div class="honeycomb-cell"></div>
  </div>
  <div class="honeycomb-row">
    <div class="honeycomb-cell"></div>
    <a class="honeycomb-cell honeycomb-omnimaga" href="https://www.omnimaga.org/" title="Omnimaga">
      <img class="honeycomb-honey" src="/bin/images/omnimaga.png" alt="Omnimaga" />
    </a>
    <div class="honeycomb-cell"></div>
    <div class="honeycomb-cell"></div>
    <a class="honeycomb-cell honeycomb-clrhome" href="https://clrhome.org/" title="ClrHome">
      <img class="honeycomb-honey" src="/bin/images/clrhome.png" alt="ClrHome" />
    </a>
    <div class="honeycomb-cell"></div>
    <div class="honeycomb-cell"></div>
    <a class="honeycomb-cell honeycomb-1064" href="https://github.com/1064CBread" title="1064&deg;C Bread">
      <img class="honeycomb-honey" src="/bin/images/1064.png" alt="1064&deg;C Bread" />
    </a>
    <div class="honeycomb-cell"></div>
  </div>
  <div class="honeycomb-row">
    <div class="honeycomb-cell honeycomb-light"></div>
    <div class="honeycomb-cell"></div>
    <div class="honeycomb-cell"></div>
    <a class="honeycomb-cell honeycomb-caltech" href="https://caltech.edu/" title="Caltech">
      <img class="honeycomb-honey" src="/bin/images/caltech.png" alt="Caltech" />
    </a>
    <div class="honeycomb-cell honeycomb-light"></div>
    <div class="honeycomb-cell"></div>
    <a class="honeycomb-cell honeycomb-blacker" href="https://blacker.caltech.edu/" title="Blacker Hovse">
      <img class="honeycomb-honey" src="/bin/images/blacker.png" alt="Blacker Hovse" />
    </a>
    <div class="honeycomb-cell"></div>
    <div class="honeycomb-cell"></div>
  </div>
  <div class="honeycomb-row">
    <div class="honeycomb-cell honeycomb-light"></div>
    <a class="honeycomb-cell honeycomb-blacker" href="https://na31.org/" title="NATO">
      <img class="honeycomb-honey" src="/bin/images/nato.png" alt="NATO" />
    </a>
    <div class="honeycomb-cell"></div>
    <div class="honeycomb-cell honeycomb-light"></div>
    <a class="honeycomb-cell honeycomb-ri" href="https://ri.cmu.edu/" title="Robotics Institute">
      <img class="honeycomb-honey" src="/bin/images/ri.png" alt="Robotics Institute" />
    </a>
    <div class="honeycomb-cell honeycomb-light"></div>
    <div class="honeycomb-cell"></div>
    <div class="honeycomb-cell honeycomb-light"></div>
    <div class="honeycomb-cell"></div>
  </div>
</div>
<p class="lead">
  <span class="drop">Hi.</span>
  <span>I'm Will Yu, computer vision engineer, avid hitchhiker, and student of life who drinks <strong>tea</strong> and poops <strong>code</strong>. I like music, krav maga, calligraphy, and <strong>opportunities</strong> to build things, climb things, or spin things. I do <a href="https://fishbotwilleatyou.com/blog/category/stupid-hacks/">stupid things</a> with <a href="/css/">CSS</a> and <a href="https://www.polygon.com/gaming/2012/9/28/3422822/fruit-ninja-gets-all-scientific-on-this-ti-83-plus-calculator">calculators</a>.</span>
</p>
