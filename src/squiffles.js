function arc(x0, y0, x1, y1, b) {
  ...
  var y = y1 - y0;
  var transform =
    'rotate(' + Math.round(Math.atan(y / x) * 180 / Math.PI) + 'deg)';
  ...
  return (
    ...
    Math.round(b * 20) / 10 +
    'em; top: ' +
    Math.round((y0 + y1) * 5 - b * 10) / 10 +
    'em; left: ' +
    Math.round((x0 + x1) * 5 - a * 10) / 10 +
    'em; -webkit-transform: ' +
    transform +
    '; -o-transform: ' +
    transform +
...
window.onload = function() {
  ...
  ReactDOM.render(
    <Showcase imageData={showcaseImageData} />,
  ...
  //~ var svg = new Svg($('#fishbot div'));
  //~ var i = 0;

  //~ showcase.load();

  //~ svg.render('/lib/img/fishbot.svg');
